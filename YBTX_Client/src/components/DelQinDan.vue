...
function handleReback(rowData) {
    let id = rowData.row.id;
    fetch(`${SERVER_URL}/api/markdel/JcbdModel/${id}?val=0`).then(async function(resp) {
        let rs = await resp.json();
        if (rs.code == 0) {
            ElNotification({ title: '成功', message: '恢复成功', type: 'success', });
        } else {
            ElNotification({ title: '失败', message: '恢复失败', type: 'error', });
        }
    });
}
    ...
    <ElTable v-if="! error" :data = 'datas' border size='default' v-bind="$attrs"  ref="qinDanRef" :height="qinDanHeight" style="width:100%; height:100%">
        ...
        <el-table-column property="op" label="操作" width="100"  fixed="right" >
            <template #default="scope">
                ...
                <el-button link type="primary" size="small" @click="handleReback(scope)" v-if="isAdmin">恢复</el-button>
            </template>
        </el-table-column>
    </ElTable>
